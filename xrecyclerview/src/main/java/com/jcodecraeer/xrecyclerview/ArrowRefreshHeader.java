...
public class ArrowRefreshHeader extends LinearLayout implements BaseRefreshHeader {
    ...
    public ArrowRefreshHeader(Context context) {
        ...
        pref = context.getSharedPreferences("lastRefreshTime",Context.MODE_PRIVATE);

	}

    /**
     * Activity运行destory周期时，将最近一次刷新时间存储。
     */
    @Override
    protected void onDetachedFromWindow() {
        ...
        if (time != null) {
            ...
            editor.putLong("time", time.getTime());
        }
    }

    /**
     * 绘制控件时，读取(如果有)上一次刷新时间
     */
    @Override
    protected void onAttachedToWindow() {
        super.onAttachedToWindow();
        if (pref.getLong("time",0) != 0 ) {
            time = new Date(pref.getLong("time",0));
        }
    }
